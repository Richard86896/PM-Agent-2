...
智目以跨境电商为核心业务（亚马逊、eBay、速卖通、B2C 独立站），同时布局国内渠道（淘宝/京东）、华强北线下门店，并通过国内外展会拓展 B2B 客户；核心产品为 FPV 无人机，研发制造到全球售后均自营。业务线广、系统分散、AI 应用近乎空白。**本岗位以跨境业务赋能为首要切入点，国内渠道与线下场景滚动跟进。**
...
| 采购计划部门 | 跨境长周期补货变量多，人工预测难系统化，库存积压无预警，大促备货难量化 | SKU 销量预测、积压预警 + 处置建议、大促备货规划 |
...
| **DeepSeek V4-Flash / V4-Pro** | 深度求索 | **官方 API（国内直连，无需翻墙）** / 私有化部署 | **中文内容生产主力**：Listing 文案、运营内容、差评回复、广告文案；成本约为 GPT-5.5 的 1/10；私有化版本适合财务/客户等敏感数据不出境场景 |
...
| 场景 | 推荐方案 | 原因 |
|------|---------|------|
| 产品 Listing / 营销文案（中文）| **DeepSeek V4-Pro** | 中文语感最优，成本极低，国内直连 |
| 产品 Listing / 营销文案（英文多语言）| Claude Sonnet 4.6 润色 | 语气地道，品牌调性一致 |
...
- 接入 Azure OpenAI（GPT-5.5、o4-mini）、Anthropic API（Claude Opus 4.7 / Sonnet 4.6）、Google AI（Gemini 3.5）、**DeepSeek 官方 API**（国内直连，中文内容主力）
...
- **数据倒推**：向 ERP / CRM 要数据，找出工单量最大的问题类型、处理时间最长的业务环节、返工率最高的流程——数据不会骗人，比访谈更客观
- **员工自建工具排查**：找到已经自建 Excel 宏、微信话术库、复制粘贴模板的员工，他们用脚投票证明了痛点，这些地方就是 AI 最值得接管的地方
- **跟着一单走**：从选品 → 上架 → 备货 → 售后跟完一个完整订单，标出每一个人工触点，发现跨部门"接缝问题"
- **同行对标**：研究同规模跨境电商已验证的 AI 落地场景，不从零发现，直接拿已跑通的案例来问"你们有没有这个问题"

**主动收集**

- **问卷调查**：全员填写"最重复的 3 件事 + 最想消失的 1 件事"，覆盖面广，低打扰
- **焦点访谈**：跨部门小组讨论，挖隐性需求和跨团队协作痛点
- **带 Demo 激发需求**：带着真实 AI 工具 Demo 去演示，问"这个能帮到你吗"——看到工具后员工才会产生具体反应

**深度验证**

- **影子跟访**：缩减到 5–6 个"说不清楚、数据看不出来"的关键岗位，坐到旁边观察真实工作流

**覆盖对象**（12 个部门/角色）：

国际销售、亚马逊运营、eBay 运营、速卖通运营、B2C 独立站、售后客服、采购计划部门、跨境物流、财务/VAT、无人机测试、视频剪辑、华强北门店

**输出物**：《AI 场景优先级矩阵》（影响规模 × 实施难度双轴排序），每个场景附真实痛点来源（数据 / 问卷 / 访谈 / 跟访）

---

### 第二阶段（第 31-60 天）：启动核心试点·完成轻量验证

> **试点 A / C**（售后客服、库存预测）需要 IT 配合，本阶段目标是**跑通流程**，ROI 数据在第三阶段量化。  
> **试点 B / D / E**（内容流水线、WhatsApp 助手、报关 OCR）由我独立完成，1–2 周内见效，本阶段完成初步数据。
...
**预期数据**：库存积压金额 ↓20%，断货率 ↓40%，补货决策时间 ↓60%
...
### 第三阶段（第 61-90 天）：跑出 ROI·推广扩散

- **核心试点 ROI 量化**：售后客服、库存预测效率提升 / 成本节省数据输出
- 五个场景前后效率对比报告（附数据对比）
- 启动分层 AI 培训（见第六节）
...
| 差评智能回复 | 🔧 工作流 | 情绪识别 + DeepSeek / Claude 生成 | 差评处理速度 ↑5× |
| A+ 内容 / 详情页设计辅助 | 🔧 工作流 | GPT Image 2 + Ideogram | 视觉内容生产成本 ↓50% |
| 竞品监控周报 | 🔧 工作流 | 数据抓取 + AI 分析摘要 | 每周自动推送竞品动态 |
| 广告文案 A/B 测试 | 🔧 工作流 | DeepSeek / Claude 批量生成变体 | 广告 CTR 持续优化 |
...
| SEO 内容批量生产 | 🔧 工作流 | 关键词 + DeepSeek / Claude 长文生成 | 自然流量↑ |
...
| 工具/场景 | 实施方式 | 方案 | 核心价值 |
|---------|---------|------|---------|
| 产品视频脚本生成 | 🔧 工作流 | DeepSeek / Claude + 产品卖点结构模板 | 脚本创作时间 ↓50% |
| AI 视频素材生成 | 🔌 接入工具 | Kling 3.0 / **Seedance 2.0**（字节，音视频联合生成）/ Vidu 2.0 | 减少实拍成本，快速出素材 |
| 产品场景图/封面图 | 🔌 接入工具 | FLUX 1.1 Pro / Midjourney v7 | 设计外包成本↓ |
| 声音克隆 + 多语言配音 | 🔌 接入工具 | **ElevenLabs**（声线克隆）| 一次录音克隆品牌声线，批量生成多语言配音，无需重复录音 |
...
| 入职 60 天 | ① 2 个核心试点（售后客服、库存预测）跑通流程 ② 3 个轻量验证（内容流水线、WhatsApp 助手、报关 OCR）完成初步效率数据 |
| 入职 90 天 | ① 核心试点 ROI 数据量化输出（效率提升 / 成本节省） ② 5 个场景前后效率对比报告 ③ 全员培训首轮覆盖率 >80% ④ 内部 AI 知识库上线，录入 ≥20 个案例 |
...
智目的业务版图——跨境多平台电商、FPV 无人机研发制造、全球售后、国内外展会——是 AI 赋能价值最高的业务组合之一。每一个环节都有大量重复劳动等待 AI 替代，每一个部门都有数据沉淀等待 AI 激活。
